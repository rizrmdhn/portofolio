...
import { createEducationSchema } from '@portofolio/schema/education.schema'
...
function RouteComponent() {
  ...
  const createMutation = useMutation(
    trpc.education.create.mutationOptions({
      onSuccess: async () => {
  ...
  const form = useForm({
    ...
    defaultValues: {
      institution: '',
      degreeLevel: 'bachelor' as (typeof DEGREE_TYPES)[number],
      major: '',
  ...
  return (
    <div className="flex min-h-[calc(100svh-var(--header-height)-2rem)] flex-1 flex-col gap-4">
      <div className="flex flex-col gap-1">
        <h2 className="text-foreground text-base font-bold">Add Education</h2>
        <p className="text-muted-foreground text-xs">Add a new education entry to your resume.</p>
      </div>
