...
export const Route = createFileRoute("/(core)/dashboard/tech-stack/create")({
  component: RouteComponent,
});

type ItemField = {
  _id: string; // local key for dnd — not the db id
  name: string;
...
function RouteComponent() {
  ...
  const createMutation = useMutation(
    trpc.techStack.createCategoryWithItems.mutationOptions({
      onSuccess: async () => {
    ...
    <div className="flex min-h-[calc(100svh-var(--header-height)-2rem)] flex-1 flex-col gap-4">
      <div className="flex flex-col gap-1">
        <h2 className="text-base font-bold text-foreground">
          New Tech Stack Category
        </h2>
        <p className="text-xs text-muted-foreground">
          Create a new tech stack category and add items to it.
        </p>
      </div>
