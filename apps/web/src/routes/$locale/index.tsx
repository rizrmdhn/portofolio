    ...
    const [profile, projects, experiences, stack, certifications, socialLinks, seo] =
      await Promise.all([
        ...
        context.queryClient.ensureQueryData(
          context.trpc.project.getForLandingPage.queryOptions({ locale }),
        ),
        context.queryClient.ensureQueryData(
          context.trpc.experience.getAll.queryOptions({ locale }),
        ),
    ...
    <div className="bg-background text-foreground flex flex-col">
        ...
        <FadeIn className="border-border mx-auto flex w-full flex-col gap-6 self-stretch border-b px-4 pb-24 md:max-w-175 md:px-0">
          {profile.availabilityStatus !== 'unavailable' && (
            ...
            >
              ...
              {t.availability[profile.availabilityStatus]}
            </Badge>
          )}
      ...
      >
        <FadeIn className="mx-auto flex w-full flex-col justify-center gap-8 px-4 md:max-w-175 md:px-0">
          <SectionHeading>{t.home.experienceHeading}</SectionHeading>
      ...
      >
        <FadeIn className="mx-auto flex w-full flex-col justify-center gap-8 px-4 md:max-w-175 md:px-0">
          <SectionHeading>{t.home.projectsHeading}</SectionHeading>
      ...
      <div className="flex flex-col items-center justify-center gap-24 py-12">
        ...
        >
          <FadeIn className="flex flex-col gap-8">
            <SectionHeading>{t.home.stackHeading}</SectionHeading>
        ...
        >
          <FadeIn className="mx-auto flex w-full flex-col justify-center gap-8 px-4 md:max-w-175 md:px-0">
            <SectionHeading>{t.home.certificatesHeading}</SectionHeading>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_1fr]">
            ...
            {isMoreCerts && (
              <Button
                onClick={() => navigate({ to: '/$locale/certificates', params: { locale } })}
                variant="link"
                size="lg"
                className="text-subtle group"
              >
        ...
        >
          <FadeIn className="flex flex-col gap-8">
            <SectionHeading>{t.home.contactHeading}</SectionHeading>
            <div className="border-border flex flex-col gap-8 rounded-lg border p-8 sm:flex-row sm:items-start sm:justify-between">
              <div className="flex flex-col gap-4">
                <p className="text-foreground text-lg font-semibold">{t.home.contactTitle}</p>
                <p className="text-subtle max-w-sm leading-relaxed">{t.home.contactBody}</p>
                <Button
                  size="lg"
                  className="w-fit"
                  onClick={() =>
